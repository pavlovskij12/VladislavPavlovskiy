      ...
      if (data.rooms?.join?.[this.roomId]) {
        ...
        roomData.timeline?.events?.forEach(event => {
          if (event.type === 'm.room.message' && !this.messages.find(m => m.id === event.event_id)) {
            this.messages.push({ id: event.event_id, body: event.content.body, sender: event.sender });
          }
        });
      }
      if (data.rooms?.invite) {
        for (const [room] of Object.entries(data.rooms.invite)) await this.joinRoom(room);
      }
